...
class DiGraph[N: Hashable](Graph[N]):
    ...
    def reverse(self,copy:bool=...)->Self:...
    def predecessors(self,n:N)->Iterator[N]:...
...
def prefix_tree(paths: Iterable[list[Any]]) -> DiGraph[int]: ...
def to_dict_of_lists[N](G: Graph[N], nodelist: list[N] = ...) -> dict[N, list[N]]: ...
def strongly_connected_components[N](G: DiGraph[N]) -> Iterable[set[N]]: ...
def relabel_nodes[
    N, V
](G: Graph[N], mapping: dict[N, V], copy: bool = ...) -> Graph[N | V]: ...
def union[N: Graph[Any]](G: N, H: N) -> N: ...


compose = union
union_disjoint = union


def union_all[N: Graph[Any]](graphs: Iterable[N]) -> N: ...


compose_all = union_all
union_disjoin_all = union_all
